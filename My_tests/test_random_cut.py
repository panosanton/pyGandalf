...
Same as test_cutting_simulation.py but the cut plane angle and depth are
randomised at startup so each run is different.

Randomised parameters (printed at startup):
    cut_plane_normal  — random unit vector (any cutting angle)
    cut_plane_origin  — center + uniform offset in [-0.7, 0.7] along normal
    blade_travel_dir  — vector in the cut plane, perpendicular to normal
...
def _random_cut_plane(rng: np.random.Generator, depth_range: float = 0.7):
    """
    Return (normal, origin, blade_dir) for a random cut through the sphere.

    normal    — random unit vector
    origin    — sphere center + uniform offset in [-depth_range, depth_range]
                along the normal (guarantees the plane intersects the sphere)
    blade_dir — unit vector in the cut plane, perpendicular to normal
    """
    # Random unit normal via spherical Gaussian
    normal = rng.standard_normal(3).astype(np.float32)
    normal /= np.linalg.norm(normal)

    # Random depth offset along the normal
    offset = rng.uniform(-depth_range, depth_range)
    origin = (normal * offset).astype(np.float32)

    # Blade direction: perpendicular to normal via Gram-Schmidt
...
def main():
    ...
    print("=" * 50)
    print("Randomised cut parameters:")
    ...
    Application().create(OpenGLWindow('Taichi — Random Cut', 1280, 720, True), OpenGLRenderer)
