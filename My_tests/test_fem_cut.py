"""
Taichi FEM Simulation — Randomised Blade Cut

Same scene as test_random_cut.py but uses FEMMethod (corotational FEM,
implicit Euler, CG solver) instead of the spring-mass backend.

FEM params:
    young_modulus  — stiffness in Pa (default 5e4)
    poisson_ratio  — compressibility (0.4 = rubber-like)
    density        — kg/m³, distributed by tet volume
    time_step      — frame dt (implicit solver: larger dt is fine)
    cg_iters       — max CG iterations per step
...
def main():
    ...
    print("=" * 50)
    print("FEM simulation — cut parameters:")
    ...
    initial_colors  = np.tile([0.3, 0.5, 0.8], (len(tet_mesh.vertices), 1)).astype(np.float32)
    ...
    # FEM-specific params merged into the standard params dict.
    # TaichiSimulationComponent passes the whole dict to method.initialize();
    # FEMMethod reads only its keys (young_modulus, poisson_ratio, density, cg_iters).
    taichi_comp = TaichiSimulationComponent(
        tet_mesh,
        time_step             = 0.01,    # implicit solver — larger dt is stable
        substeps              = 1,       # no substeps needed for implicit FEM
        stiffness             = 200.0,   # cutting spring stiffness
        damping               = 2.0,
        total_mass            = 100.0,
        gravity               = [0.0, 0.0, 0.0],
        opening_speed         = 5.0,
        ...
        method_instance       = FEMMethod(cg_iters=args.cg_iters),
    )
    ...
    # Inject FEM-specific keys by monkey-patching the component's param dict after
    # on_create_entity runs — or pre-set them so on_create_entity picks them up.
    ...
    print("Starting FEM simulation.  Press B to cut.  Close window to exit.")
    Application().start()
